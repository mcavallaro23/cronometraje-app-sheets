...
const timeToSeconds = (timeStr) => {
    ...
    const simulatePhotocellFrame = (channel) => {
        ...
        // Buscar qué dispositivo disparó (F1, F7, F9, etc)
        let deviceName = '';
        for (const [key, device] of Object.entries(bleDevices)) {
            if (device.connected) {
                // Verificar si este dispositivo puede enviar este canal
                const deviceChannel = key.substring(1);
                if (deviceChannel === channel.toString()) {
                    deviceName = key;
                    break;
                }
            }
        }
        if (!deviceName) deviceName = `F${channel}`; // Fallback
        ...
        if (channelLastFired[channel]) {
            ...
            if (timeSinceLastFire < DEAD_TIME) {
                ...
                if (semaforo && semaforoText) {
                    ...
                    // Apagar después del tiempo restante
                    setTimeout(() => {
    ...
    const updateAthleteSelect = () => {
        ...
        if (selectedAthlete) {
            document.getElementById('current-athlete').textContent = selectedAthlete;
        }
    };
...
startBtn.addEventListener('click', () => {
    ...
    if (buttonText === "START") {
        // Plegar persiana al arrancar test
        if (!isCollapsed) {
            selectionDropdowns.classList.add('collapsed');
            expandBtn.textContent = '▲';
            isCollapsed = true;
...
// Event listener para clicks en tarjetas
document.querySelectorAll('.device-card').forEach(card => {
    card.addEventListener('click', async () => {
        const deviceKey = card.dataset.device;
        const device = bleDevices[deviceKey];
        
        // Si no existe el device o se perdió la referencia, intentar recuperarlo
        ...
        // Intentar conectar desde estado desconectado
            try {
                ...
                characteristic.addEventListener('characteristicvaluechanged', (event) => {
                    ...
                    // Si es una trama válida, disparar fotocélula
                    ...
                    if (frameMatch) {
                        ...
                        if (channel === expectedChannel) {
                            ...
                            // AGREGAR: Flash visual en PHOTOCELLS
                            if (currentPage === 'page-photocells') {
                                const card = document.querySelector(`[data-device="${deviceKey}"]`);
                                if (card) {
                                    card.style.backgroundColor = '#00ff00'; // Verde brillante
                                    card.style.boxShadow = '0 0 20px #00ff00';
                                    setTimeout(() => {
                                        card.style.backgroundColor = '#1a3a1a'; // Volver al verde normal
                ...
                device.gattHandler = () => {
                    ...
                    device.connected = false;
                    updateDeviceCard(deviceKey, 'disconnected', 'DISCONNECTED');
                    // Mantener en scanningDevices para poder reconectar
                    scanningDevices.add(deviceKey);
                    if (!document.hidden) {
                        alert(`⚠️ Fotocélula ${deviceKey} perdió conexión`);
                    }
                };
                ...
                // Mantener estado disconnected si ya estaba desconectado
                if (device.connected === false) {
                    updateDeviceCard(deviceKey, 'disconnected', 'DISCONNECTED');
                } else {
                    updateDeviceCard(deviceKey, 'disconnected', 'DISCONNECTED');
                }
                alert(`Error conectando ${deviceKey}`); 
            }
        }
    });
});
